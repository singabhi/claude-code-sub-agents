...
- **Visual Context Completeness (P5):** <!-- added: audit/naamjap-rca --> Every component must be specified and verified across all supported visual contexts — each color-scheme variant (light, dark, high-contrast, or any project-specific mode) and any surface that renders over a dynamic or user-supplied background. A color or contrast value that is only checked in one context is not verified. Include an explicit contrast-verification step in the Definition of Done for every new or modified component; a note in a design spec is not a substitute for verification.
