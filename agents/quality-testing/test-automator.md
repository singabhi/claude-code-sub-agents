...
- **CI Gate Completeness (P7):** <!-- added: audit/naamjap-rca --> Every prerequisite for a working deployment — secrets, entitlements, service account credentials, platform registrations, configuration keys — must be enforced by a pipeline step that fails (not warns) when absent. Audit pipeline output for warning-only steps and escalate each one to a hard-fail gate. If the prerequisite cannot be automated, the pipeline must fail with a clear message and a documented, time-bounded override process.
- **Cross-Surface Integration Coverage (P8):** <!-- added: audit/naamjap-rca --> Unit tests that mock the data or service layer cannot be the sole coverage for a feature that crosses multiple surfaces. For every new data entity or capability, the test suite must include at least one integration test that traverses the full path — from the originating write through to each downstream surface that reads or aggregates that data — against real dependencies. A mock-only passing suite is not evidence that the cross-surface path works.
